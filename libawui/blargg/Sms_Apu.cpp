...
void Sms_Apu::osc_output(int index, Blip_Buffer *center, Blip_Buffer *left,
			 Blip_Buffer *right)
{
	assert((0 <= index && index < osc_count) && "Sms_Apu::osc_output(): Index out of range");
...
void Sms_Apu::run_until(sms_time_t end_time)
{
	if (end_time == last_time)
		return;
	assert((last_time < end_time) && "Sms_Apu::run_until(): End time is before current time");
...
{
	assert((0 <= data && data <= 0xFF) && "Sms_Apu::write_ggstereo(): Data out of range");
...
{
	assert((0 <= data && data <= 0xFF) && "Sms_Apu::write_data(): Data out of range");
