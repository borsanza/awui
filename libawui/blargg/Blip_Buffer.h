...
{
	offset_ += t * factor_;
	assert((samples_avail() <= buffer_size_) && "Blip_Buffer::end_frame(): Frame went past end of buffer");
}

inline void Blip_Buffer::remove_silence(size_t count)
{
	assert((count <= samples_avail()) && "Blip_Buffer::remove_silence(): Not that many samples available to remove");
