...
bool Blip_Buffer::sample_rate(long sps, int msec)
{
	...
	assert((new_size <= max_size) && "Blip_Buffer::sample_rate(): Buffer length exceeds limit");
...
void Blip_Buffer::clock_rate(long cps)
{
	...
	assert((factor_ > 0) && "Blip_Buffer::clock_rate(): Clock rate to sample rate ratio exceeded 65536");
}
...
{
	assert(buffer_ && "Blip_Buffer::read_samples(): Buffer sample rate not set");
